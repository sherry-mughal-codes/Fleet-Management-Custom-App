...
def get_data_and_summary(filters):
	...
	for v in vehicles:
		...
		if status in ["Available"]:
			available_cnt += 1
		elif status in ["Under Maintenance", "Maintenance Due"]:
			maintenance_cnt += 1
		...
		data.append({
			...
			"current_odometer": float(v.get("current_odometer") or 0.0),
			...
			"cost_per_km": cost_km,
			"next_maintenance_due_odometer": float(v.get("next_maintenance_due_odometer") or 0.0)
		})
