...
class MaintenanceTemplate(BaseFleetDocument):
	...
	def validate(self):
		...
		for row in getattr(self, "vehicles", []) or []:
			...
			if v_id and frappe.db.exists("Fleet Vehicle", v_id):
				curr_temp = frappe.db.get_value("Fleet Vehicle", v_id, "maintenance_template")
	...
	def on_update(self):
		...
		for v_id in target_vehicles:
			if frappe.db.exists("Fleet Vehicle", v_id):
				curr_temp = frappe.db.get_value("Fleet Vehicle", v_id, "maintenance_template")
				if curr_temp != template_name:
					frappe.db.set_value("Fleet Vehicle", v_id, "maintenance_template", template_name)
					frappe.clear_document_cache("Fleet Vehicle", v_id)

		# 2. Unlink vehicles that previously pointed to this template but were removed from child table
		linked_vehicles = frappe.get_all("Fleet Vehicle", filters={"maintenance_template": template_name}, fields=["name"]) if hasattr(frappe, "get_all") else []
		for v in linked_vehicles:
			v_name = v.name if hasattr(v, "name") else v.get("name")
			if v_name and v_name not in target_vehicles:
				frappe.db.set_value("Fleet Vehicle", v_name, "maintenance_template", None)
				frappe.clear_document_cache("Fleet Vehicle", v_name)
	...
	def on_trash(self):
		...
		if hasattr(frappe, "db") and frappe.db:
			linked = frappe.get_all("Fleet Vehicle", filters={"maintenance_template": self.name}, fields=["name"]) if hasattr(frappe, "get_all") else []
			frappe.db.set_value("Fleet Vehicle", {"maintenance_template": self.name}, "maintenance_template", None)
			for v in linked:
				v_name = v.name if hasattr(v, "name") else v.get("name")
				if v_name:
					frappe.clear_document_cache("Fleet Vehicle", v_name)
