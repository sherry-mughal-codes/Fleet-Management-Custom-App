frappe.ui.form.on('Vehicle Assignment', {
	...
	vehicle: function(frm) {
		if (frm.doc.vehicle) {
			frappe.db.get_value('Vehicle', frm.doc.vehicle, 
				['vehicle_number', 'vehicle_name', 'vehicle_brand', 'vehicle_model', 'vehicle_category', 'fuel_type', 'current_odometer', 'company', 'status'], 
				function(r) {
					if (r) {
						...
						if (r.vehicle_category) frm.set_value('vehicle_category', r.vehicle_category);
						if (r.fuel_type) frm.set_value('fuel_type', r.fuel_type);
						if (r.current_odometer !== undefined) {
							frm.set_value('current_odometer', r.current_odometer);
							if (!frm.doc.opening_odometer) {
								frm.set_value('opening_odometer', r.current_odometer);
							}
						}
						if (r.company && !frm.doc.company) frm.set_value('company', r.company);
						if (r.status) frm.set_value('vehicle_status_indicator', r.status);
					}
				}
			);
		}
	...
	employee: function(frm) {
		if (frm.doc.employee) {
			frappe.db.get_value('User', frm.doc.employee, ['full_name'], function(r) {
				if (r && r.full_name) {
