frappe.ui.form.on('Maintenance Request', {
	vehicle: function(frm) {
		if (frm.doc.vehicle) {
			var fetch_fields = ['vehicle_number', 'vehicle_name', 'vehicle_brand', 'vehicle_model', 'current_odometer', 'company', 'status', 'last_maintenance_date'];
			
			// Safely check if last_maintenance_odometer is a permitted field in current meta
			if (frappe.get_meta('Vehicle') && frappe.meta.has_field('Vehicle', 'last_maintenance_odometer')) {
				fetch_fields.push('last_maintenance_odometer');
			}

			frappe.db.get_value('Vehicle', frm.doc.vehicle, fetch_fields, function(r) {
				if (r) {
