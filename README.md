...
- **Cataloged Validation Rules**: Rule IDs (`VEH-001` .. `VEH-010`, `ASSET-001` .. `ASSET-008`, `ASSIGN-001` .. `ASSIGN-010`, `FUEL-001` .. `FUEL-010`, `MASTER-001` .. `MASTER-013`).
...
│   ├── api/                 # Enterprise Whitelisted API Wrappers (Vehicle, Assignment, Fuel APIs)
│   ├── business_rules/      # Decoupled Business Invariant Engine (Vehicle, Assignment, Fuel Rules)
│   ├── config/              # Desk Sidebar & Module Configurations
│   ├── dashboard/           # Desk Dashboard Charts & Analytics Definitions
│   ├── events/              # Event Registry, Vehicle, Assignment & Fuel Event Dispatchers
│   ├── fleet_management/    # Desk Workspace, Master DocTypes, Vehicle, Assignment & Fuel DocTypes
│   ├── mixins/              # Reusable Document Mixins
│   ├── notifications/       # Multi-Channel Notification Engine & Service
│   ├── permissions/         # Security Evaluators (Vehicle, Assignment & Fuel Permissions)
│   ├── services/            # Base Service, SettingsService, VehicleService, AssignmentService, FuelService, FuelAverageService, MaintenanceLockService
│   ├── tests/               # Pytest Unit & Integration Test Suites
│   ├── utils/               # BaseFleetDocument, Logger, Exception Hierarchy, Helpers
│   ├── validators/          # Input, Entity, Vehicle, Asset, Assignment & Fuel Validators
